...
EndGoalObject::EndGoalObject(const glm::vec3& position, Geometry* geom, Shader* shader, GLuint texture, glm::vec2& scale, float radius)
	: GameObject(position, geom, shader, texture, scale), ColliderObject(radius) {}

bool EndGoalObject::circleCollision(ColliderObject* other) const {
...
bool EndGoalObject::rayCollision(ColliderObject* other) const {
